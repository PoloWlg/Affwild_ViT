...
class GenericVideoTrainer(GenericTrainer):
    ...
    def loop(self, **kwargs):
        ...
        if self.save_feature_maps:
            feature_maps_handler = FeatureMapsOutputHandler()
        ...
        for batch_idx, (X, trials, lengths, indices, features_video) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            for feature, value in X.items():
                # if (feature == 'context'):
                #     inputs[feature] = {}
                #     inputs[feature]['ids'] = X[feature]['ids'].to(self.device)
                #     inputs[feature]['mask'] = X[feature]['mask'].to(self.device)
                #     inputs[feature]['token_type_ids'] = X[feature]['token_type_ids'].to(self.device)
                # else:
                #     inputs[feature] = X[feature].to(self.device)
                inputs[feature] = X[feature].to(self.device)

            if "EXPR_continuous_label" in inputs:
                labels = inputs.pop("EXPR_continuous_label", None)
            elif "EXPR_continuous_label" in inputs:
                labels = inputs.pop("EXPR_continuous_label", None)
            ...
            # outputs = outputs.unsqueeze(2).repeat(1,1,300)
            # labels_new = torch.mean(labels, 1, keepdim=True, out=None).squeeze(2).unsqueeze(0)
            ...
            # labels = labels.unsqueeze(1).repeat(1, 300, 1)
            # outputs = outputs.squeeze(0).unsqueeze(1).repeat(1, 300, 1)
            ...
            if train_mode:
                loss.backward()
                for name, param in self.model.named_parameters():
                    # if name.startswith('inter_connexions.'):
                    #     print(param.mean())
            ...
            # Update the features handler
            if False:
                features_handler.update_output_for_seen_trials(features, trials, indices, lengths)
            
            if self.compute_att_maps and self.save_feature_maps:
                feature_maps_handler.update_output_for_seen_trials(trials, indices, lengths, self.model)
        ...
        for key in features_handler.trialwise_records:
            ...
            if train_mode:
                ...
                label = 'val'
            np.save(os.path.join('/projets/AS84330/Datasets2/Abaw6_EXPR_contextual/features','raw_video_features', label, key + ".npy"), features_handler.trialwise_records[key])
        ...
        all_labels = continuous_label_handler.partition_records
        
        if self.compute_att_maps and self.save_feature_maps:
            feature_maps_handler.average_trial_wise_records()
            feature_maps_handler.concat_records()
            feature_maps_handler.get_feature_maps(all_labels, self.save_path)
