...
class VisualBackbone2(nn.Module):
    def __init__(self, input_channels=3, num_classes=8, use_pretrained=True, state_dict_path="", mode="ir",
                 embedding_dim=512):
        super().__init__()
        self.backbone = Backbone(input_channels=input_channels, num_layers=50, drop_ratio=0.4, mode=mode, drop_last_layer=True)
        if use_pretrained:
            state_dict = torch.load(state_dict_path, map_location='cpu')
...
class ContextBackbone(nn.Module):
    ...
    def forward(self, feats):
        x = self.roberta(input_ids=feats['ids'], attention_mask=feats['mask'], token_type_ids=feats['token_type_ids'])
        hidden_state = x[0]
